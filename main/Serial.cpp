...
static const T_COM_STS *bt_timeout(const T_COM_STS *ptr_this);
...
/* アイドル状態 */
static const T_COM_STS c_t_com_sts_idle =
{
    bt_rcv_mark,
    bt_ignore,
    bt_st_snd,
...
/* 受信/送信状態 */
static const T_COM_STS c_t_com_sts_com =
{
    bt_ignore,
    bt_rcving,
    bt_snding,
...
/* BCC受信状態 */
static const T_COM_STS c_t_com_sts_bcc =
{
    bt_ignore,
    bt_sp_rcv,
    bt_sp_snd,
...
static void bt_vdOnTo(void)
{
    ptr_com_sts = ptr_com_sts->vdTo(ptr_com_sts);
};
...
void BT_vdInit(void){
    s_t_bt.u1_wait_1ms_pef_cnt = 0;
    s_t_bt.u1_timeout_500ms_per_cnt = 0;
    s_t_bt.u1_sts = 0;
...
void BT_vdWait20ms(void)
{
    static uint16_t s_u2_cnt = 0;
    s_u2_cnt++;
    return;
}

static bool bt_bProcess(void)
{
    /* 受信 */
    if(HIGH == digitalRead(INPUT_PORT))
    {
...
{
    if(s_t_bt.u1_wait_1ms_pef_cnt)
        s_t_bt.u1_wait_1ms_pef_cnt--;
    return;
}
void BT_vdDecrementTimeout(void)
{
    if(s_t_bt.u1_timeout_500ms_per_cnt)
        s_t_bt.u1_timeout_500ms_per_cnt--;
    return;
}
...
void BT_vdMain(void){
    if(0 == s_t_bt.u1_wait_1ms_pef_cnt)
    {
        if(bt_bProcess())
        {
            s_t_bt.u1_wait_1ms_pef_cnt = 20;        /* 20[ms] */
            s_t_bt.u1_timeout_500ms_per_cnt = 20;   /* 10[s] */
        }
    }
        /* タイムアウト判定 */
    if(0 ==s_t_bt.u1_timeout_500ms_per_cnt)     /* タイムアウトの場合 */
    {
        bt_vdOnTo();
    }
    return;
...
UCHAR BT_u1Wait20msCnt(void){return s_t_bt.u1_wait_1ms_pef_cnt;}
UCHAR BT_u1Timeout500msCnt(void){return s_t_bt.u1_timeout_500ms_per_cnt;}
UCHAR BT_u1Sts(void)
{
    if(ptr_com_sts == &c_t_com_sts_idle) return 0;
    if(ptr_com_sts == &c_t_com_sts_com)  return 1;
    if(ptr_com_sts == &c_t_com_sts_bcc)  return 2;
    return 0xFF;
}